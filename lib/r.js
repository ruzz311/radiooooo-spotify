...
async function getSongsByCountry(count, moods, decade, country) {
  ...
  async function recurse() {
    ...
    if (!songExistsInList) {
      songs.push(song);
      if (recursionCount <= count) await recurse();
    }
  }
...
async function getSongsByRegion(count, moods, decade, region, supportedCountryCodes) {
  ...
  let country = util.getRandomCountryCodeFromRegion(region, supportedCountryCodes);
  const playlistId = await getPlaylistId();

  async function recurse() {
    recursionCount += 1;
    country = util.getRandomCountryCodeFromRegion(region, supportedCountryCodes);
    ...
    try {
      ...
      // ignore errors and proceed, but die if infinitent recursion is possible
      if (recursionCount >= count * 2) {
        throw new Error('max recursion count reached');
      }
      await recurse();
    }
  }
