...
async function refreshAccessToken(refreshToken) {
  ...
  return needle('post', config.SPOTIFY.TOKEN_URL, {
    ...
    if (res.statusCode < 300 && res.body && res.body.access_token) {
      return store.set('spotify_access_token', res.body.access_token);
    }

    // if the refresh token fails, go create a new access token
    // delete the existing refresh_token and the access_token
    await store.set('spotify_access_token', null);
    await store.set('spotify_refresh_token', null);
    return setAccessToken(); // eslint-disable-line no-use-before-define
  });
}
